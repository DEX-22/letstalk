...
const matcher = /* @__PURE__ */ (() => {
  const $0 = { ssr: false, prerender: true }, $1 = { ssr: false };
  return (m, p) => {
    let r = [];
...
function useHead(input, options = {}) {
  const head2 = options.head || injectHead(options.nuxt);
...
const supabase_server_LD3l3PlxdJt1kDAI6eTyRYyLVnXt3NZgjHOrAos_ias = /* @__PURE__ */ defineNuxtPlugin({
  name: "supabase",
  enforce: "pre",
...
const auth_redirect_vkXibr14SCSk3LrpDKYigdtt3KQPwIskUL_aVcdXAss = /* @__PURE__ */ defineNuxtPlugin({
  name: "auth-redirect",
  setup() {
    addRouteMiddleware(
      "global-auth",
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<button${ssrRenderAttrs(mergeProps({
        ...
        class: unref(cn)(unref(buttonVariants)({ variant: props.variant, size: props.size }))
      }, _attrs))}>`);
      if (props.loading) {
        _push(`<span class="h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent"></span>`);
      } else {
        _push(`<!---->`);
      }
...
_sfc_main$2.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("shared/components/ui/Button.vue");
  return _sfc_setup$2 ? _sfc_setup$2(props, ctx) : void 0;
};
const __nuxt_component_0 = Object.assign(_sfc_main$2, { __name: "Button" });
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "flex min-h-dvh flex-col items-center justify-center gap-6 bg-slate-950 px-4" }, _attrs))}><div class="text-center"><p class="text-6xl font-bold text-primary-500">${ssrInterpolate(__props.error.statusCode)}</p><h1 class="mt-4 text-xl font-semibold text-slate-100">${ssrInterpolate(__props.error.statusMessage ?? "Something went wrong")}</h1>`);
      if (__props.error.message) {
        _push(`<p class="mt-2 text-sm text-slate-400">${ssrInterpolate(__props.error.message)}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
